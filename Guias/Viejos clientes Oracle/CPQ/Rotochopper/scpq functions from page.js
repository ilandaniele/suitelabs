 ...
 *** Changes log ***
 ...
 that are answered, labels for answers to all answered questions. Also edited
 script to clear sales prices on all new items (had to correct the clearCosts
 section since it was clearing costs in the original item instead of the new
 item).
 5/26/2022 v2.51 JMS: Added sections to add installed screens to configured
 top-level BOM and to add their descriptions to 'Configuration Options' field.
...
window.getAdditionalBOM = async() => {
	...
	/*JMS Add to bomlist here.
	Format = 
	{question: 'MODEL', answer: 'B66', rule: 'ROTOR/24T_288', BOMLevel: '1', part_id_to_replace: '', qty: "1", part_id: "20-652", parent: "04-0823"}
	*/

	let top_item = getMasterID();
	for (let i = 1; i <= 2; i++) {	//JMS Loop through table of installed screens and add to bomlist
		let screensTableQty = getValue('SCREENS_TABLE', 'QTY_' + i);
		let screensTableCode = getValue('SCREENS_TABLE', 'CODE_' + i);
		if (screensTableCode) {	//JMS Check if this row has a screen code
			screensTableCode = screensTableCode.replace('in', '"');
			console.log("screensTableCode_row" + i + " = " + screensTableCode);
			let instScreens = {question: '', answer: '', rule: '', BOMLevel: '1', part_id_to_replace: '', qty: screensTableQty, part_id: screensTableCode, parent: top_item};
			bomlist.push(instScreens);
		}
		else {
			console.log("screensTableCode_row" + i + " is blank");
		}
	};
...
window.itemCreation = async() => {
	...
	let clearCosts = async(level) => {
		...
		let data = await getData({
			type: 'item',
			filter: ['name', 'is', level.use_itemid],	//JMS Changed to ['name', 'is', level.use_itemid] (original: ['internalid', 'is', level.id])
			fields: ['itemid'],	//JMS Added 'itemid' to grab the name field for use in the console.log command below
			sublists: {
		...
		//JMS Trying to clear sale prices in new items
		let objSales = {};	//JMS Need a new version of obj to house the sales prices
		data[0].sublists.price.map(p => {
			if (p.price_1_) {
				objSales[p.pricelevel] = {
					price_1_: ''
				};
			}
		});
		...
		await createRecord({
			...
			id: data[0].id,	//JMS Changed to data[0].id instead of level.id
			sublists: {
				'locations:set': {
					key: 'locationid',
					items: obj
	...
	let createItems = async(level) => {
		...
		if (level.top_level) {
			...
			if (!found) {
				...
				level.use_itemid = level.itemid;	//JMS Added this line so clearCosts can run
				...
				await clearCosts(level);	//JMS Added this line
			}
		}
	};
	...
	let options = [];
	let toInclude = ['ENGINE', 'DRIVE', 'VOLTAGE', 'HITCH', 'AXLES', 
		'DISCHARGE', 'COLORIZER', 'DOLLY', 'ROTOR', 'TOOTH_CONFIG', 'TOOTH', 
		'TOOTH1', 'TOOTH2', 'SCREENS_TABLE', 'CROSS', 'CB_DISC', 'ADDONS', 'ROTOLINK_CONN', 
		'NOZZLES'];	//JMS
	getRuleset('questions').map(q => {	//JMS Steps through all questions (ignoring hidden)
		if (toInclude.includes(q.code)) {	//JMS Include answers to many questions instead of just 'Additional Options'
			if (isAnswered(q.code)) {	//JMS
				if (q.code == 'SCREENS_TABLE') {	//JMS Need to do something special for installed screens
					options.push("Installed Screens:" + "\n");	//JMS
					for (let i = 1; i <= 2; i++) {	//JMS Loop through table of installed screens and add to bomlist
						let screensTableQty = getValue('SCREENS_TABLE', 'QTY_' + i);
						let screensTableStyle = getValue('SCREENS_TABLE', 'STYLE_' + i);
						if (screensTableStyle) {	//JMS Check if this row has a screen description
							screensTableStyle = screensTableStyle.replace(/in /g, '" ');
							screensTableStyle = screensTableStyle.replace(/ - /g, ', ');
							screensTableStyle = screensTableStyle.replace(/- /g, ', ');
							console.log("screensTableStyle_row" + i + " = " + screensTableStyle);
							options.push("-" + "(" + screensTableQty + ") " + screensTableStyle + "\n");
						}
						else {
							console.log("screensTableCode_row" + i + " is blank");
						}
	...
	if (config_bom.length == 0) {
		...
		if (main_item_data.length == 0) {
			...
			let level = {
				...
				use_itemid: reserved_itemid,	//JMS Added this line so clearCosts can run
			};
...
window.getPropgenFields = () => {
	...
	function getTeethOptions() {
		...
		let template = '';
			// `<p class="page-4" style="margin-top: 20px;"><b>Additional Teeth Options</b></p>
			// 	<table class="model-table page-4" id="teeth-option-header">
			// 		<tr>
			// 			<td>Description</td>
			// 			<td>Price USD</td>
			// 		</tr>
		...
		if (activeItems.length) {
			for (var i = 0; i < activeItems.length; i++) {
				...
				template += `<table class="model-table page-4">
					<tr ${style}>
						<td>${text} Additional x${qty}</td>
						<td>${price}</td>		
					</tr>
				</table>`;
			}
	...
	function getScreenOptions() {
		...
		let activeItems = [];
		let items = getRuleset('items');
		let showAdditionalPrice = getValue('SCREEN_SEL_PDF','YES');

		let template = '';
			// `<p class="page-4" style="margin-top: 20px;"><b>Additional Screens Options</b></p>
			// 	<table class="model-table page-4" id="screen-option-header">
			// 		<tr>
			// 			<td>Description</td>
			// 			<td>Price USD</td>
			// 		</tr>
