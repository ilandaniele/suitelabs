 ...
 *** Changes log ***
 ...
 7/1/2022 Ilan v2.53: Trying to show prices on PDF Printing.
...
window.getPropgenFields = () => {
	...
	function getDesc() {
		...
		const dieselMotor = 'ENGINE/'+getAnswerCode('ENGINE');
		const electricMotor = 'DRIVE/'+getAnswerCode('DRIVE');
		const Motor = dieselMotor.includes('ANS CODE NF') ? electricMotor : dieselMotor;
		// const Motor = getValue('HIDDEN', 'DRIVE_QCODE');
		const Rotor = 'ROTOR/'+getAnswerCode('ROTOR');
		const Voltage = 'VOLTAGE/'+getAnswerCode('VOLTAGE');
		...
		if (standardFeatures) {
			standardFeatures.forEach(function (ans) {
				...
				//aca buscar que texto se setee con el nuevo nombre seteado de "MOTOR" o de "ROTOR" o de "VOLTAGE" elegido arriba como constante.
				//el texto tiene que reemplazar ese 500 con el 600 HP.
