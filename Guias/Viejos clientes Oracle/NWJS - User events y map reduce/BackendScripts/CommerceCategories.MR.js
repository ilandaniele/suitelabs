...
define([
    ...
    function map(context) {
        ...
        // Obtengo el item id del record actual (column: name: 'custrecord_nssc_item')
        // Equivale al var id = result.getValue({ name: 'custrecord_nssc_item' }); dentro del forEach
                ...
                try {
                    jsonData = (jsonData).map(function each(item) {
                        if (item.urlcomponent === relatedCategoryValuesJSON.urlComponentOld) {
                            ...
                            item.img = relatedCategoryValuesJSON.image;
                        }
                        return item;
                    });
                } catch (e) {
