		...
		{{#if isPriceEnabled}}
			<div class="cart-summary-subtotal">
				<p class="cart-summary-grid-float">
                    <span class="cart-summary-amount-subtotal-title">{{translate 'Subtotal'}}</span>
					<span class="cart-summary-amount-subtotal">
                        {{#if useRetailPrices}}
                            {{ summary.retailSubTotalFormatted }}
                        {{else}}
                            {{ summary.subtotal_formatted }}
                        {{/if}}
					</span>
						{{#if isSingleItem}}
							{{translate '<span class="cart-summary-item-quantity-subtotal">$(0) RAD item</span>' itemCount}}
						{{else}}
							{{translate '<span class="cart-summary-item-quantity-subtotal">$(0) RAD items</span>' itemCount}}
						{{/if}}
				</p>
			</div>
            ...
            {{#if jewelry.active}}
                <p class="cart-summary-grid-float">
                    <span class="cart-summary-amount-subtotal">
                        {{#if useRetailPrices}}
                            {{ jewelry.jewelry_subtotal_formatted_retail}}
                        {{else}}
                            {{ jewelry.jewelry_subtotal_formatted }}
                        {{/if}}
                    </span>
                    {{#if jewelry.singleItem}}
                        {{translate '$(0) Jewelry item' jewelry.jewelry_count}}
                    {{else}}
                        {{translate '$(0) Jewelry items' jewelry.jewelry_count}}
                    {{/if}}
                </p>
                <p class="cart-summary-grid-float">
                    <span class="cart-summary-amount-subtotal">
                        {{ jewelry.flatrate_formatted }}
                    </span>
                    {{translate jewelry.text}}
                </p>
            {{/if}}
            <p>
                {{#if jewelry.active}}
                    <div class="cart-summary-subtotal-legend">
                        {{translate 'Total does not include shipping for non Jewelry Items'}}
                    </div>
                {{else}}
                    <div class="cart-summary-subtotal-legend">
        ...
        {{else}}
            <div class="cart-summary-message cart-summary-msg-description">
                <p class="cart-summary-login-to-see-price">
                    {{translate 'Please contact the company buyer to place your order.'}}
                </p>
            </div>
        {{/if}}
