<article class="overview-shipping">
	...
	{{#if hasDefaultShippingAddress}}
		...
		<div class="overview-shipping-card-button-edit-container">
			<a class="overview-shipping-card-button-edit button-secondary button-medium" href="/addressbook/{{shippingAddressInternalid}}" data-toggle="show-in-modal">{{translate 'Edit'}}</a>
		</div>
	{{else}}
		<div class="overview-shipping-card-content">
			<p>{{translate 'We have no default address on file for this account.'}}</p>
		</div>
		<a href="/addressbook/new" class="overview-shipping-card-button-edit button-secondary button-medium" data-toggle="show-in-modal">{{translate 'Create New Address'}}</a>
	{{/if}}
	</section>
</article>
