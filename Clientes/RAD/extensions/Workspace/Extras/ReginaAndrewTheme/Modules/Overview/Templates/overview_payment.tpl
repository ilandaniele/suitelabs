<article class="overview-payment">
	...
	{{#if hasDefaultCreditCard}}
		...
		<div class="overview-payment-card-button-edit-container">
			<a class="overview-payment-card-button-edit button-secondary button-medium" href="/creditcards/{{creditCardInternalid}}" data-toggle="show-in-modal">{{translate 'Edit'}}</a>
		</div>
	{{else}}
		<div class="overview-payment-card-content">
			<p>{{translate 'We have no default credit card on file for this account.'}}</p>
		</div>
		<a href="/creditcards/new" class="overview-payment-card-button-edit button-secondary button-medium" data-toggle="show-in-modal">{{translate 'Add a Credit Card'}}</a>
	{{/if}}
	</section>
</article>
