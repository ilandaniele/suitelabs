...
<div class="header-mini-cart">
	 {{#if showLines}}
		...
		<div class="header-mini-cart-subtotal">
			...
			<div class="header-mini-cart-subtotal-items">
				{{#if showPluraLabel}}
					{{translate '$(0) RAD items' radItems}}
				{{else}}
					{{translate '1 RAD item'}}
				{{/if}}
			</div>
            ...
            {{#if isPriceEnabled}}
                <div class="header-mini-cart-subtotal-amount">
                    {{#if useRetailPrices}}
                        {{translate '$(0)' retailSubTotalFormatted}}
                    {{else}}
            ...
            {{#if jewelry.active}}
                ...
                {{#if isPriceEnabled}}
                    <div class="header-mini-cart-subtotal-amount">
                        {{#if useRetailPrices}}
                            {{translate '$(0)' jewelry.jewelry_subtotal_formatted_retail}}
                        {{else}}
                            {{translate '$(0)' jewelry.jewelry_subtotal_formatted}}
                        {{/if}}
                    </div>
                {{/if}}
            {{/if}}
