{{#if showFacet}}
	<div class="facets-faceted-navigation-item-color-facet-group"  data-type="rendered-facet" data-facet-id="{{facetId}}">
		{{#if showHeading}}
			{{#if isUncollapsible}}
		...
		{{else}}
		<div class="facets-faceted-navigation-item-color-facet-group-wrapper {{#if isCollapsed}} collapse {{else}} in{{/if}}" id="{{htmlId}}-facets-faceted-navigation-item-color-facet-group-wrapper">
		{{/if}}
			<div class="facets-faceted-navigation-item-color-facet-group-content">
							...
							<a href="{{link}}" title="{{label}}" data-color="{{label}}" class="facets-faceted-navigation-item-color-option {{#if isLightColor}}white-border{{/if}} {{#if isActive}}active{{/if}}">
                                    ...
                                    {{#if isImageTile}}
                                        <img
                                                ...
                                                height="{{image.height}}">
                                    {{else}}
                                        <span  class="color-tooltip" data-toggle="tooltip" data-placement="top" title="{{label}}" style="background: {{color}}"></span>
                                    {{/if}}
							</a>
						</li>
					{{/each}}
				...
				{{#if showExtraValues}}
					<ul class="facets-faceted-navigation-item-color-picker-extra collapse" id="{{htmlId}}-facets-faceted-navigation-item-color-picker-extra">
						{{#each extraValues}}
							<li >
