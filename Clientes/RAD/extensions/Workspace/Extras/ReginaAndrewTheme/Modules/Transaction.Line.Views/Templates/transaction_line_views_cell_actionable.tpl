<tr id="{{lineId}}" data-item-id="{{itemId}}" data-type="order-item" {{#if showGeneralClass}} class="{{generalClass}}" {{/if}} >
	...
	<td class="transaction-line-views-cell-actionable-table-middle">
        ...
        {{#if expectedShipDate}}
            <p>
                <span class="transaction-line-views-quantity-amount-label">{{translate 'In-Stock Date:'}} </span>
                <span class="transaction-line-views-quantity-amount-value">{{expectedShipDate}}</span>
            </p>
        {{/if}}
	</td>
