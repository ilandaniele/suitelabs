...
function validateSuiteScriptHeaders(file) {
    ...
    // prettier-ignore
    // prettier is changing this code to @NApiVersion 2.x
    // this is a regex, so it is dangerous if the string is changed
...
gulp.task('suitescript', cb => {
    const backend_path = path.join(package_manager.env.srcDir, '..');
        ...
        .pipe(
            ...
            through(function(file, type, done) {
                ...
                src_file_name = path
                    .join(
                        `${backendDestFolder}`,
                        path.relative(path.join(file.base, 'Backend'), file.path)
                    )
                    .replace(/\\/g, '/')
                    .replace(/\.ts$/, '');
        ...
        .pipe(
            ts({
                ...
                paths: {
                    ...
                    'N/crypto': ['../Backend/SCIS/Crypto/crypto.d.ts']
                }
            })
        )
        .pipe(
...
gulp.task('third_parties', cb => {
    ...
    return gulp.src(third_parties, { allowEmpty: true }).pipe(gulp.dest(dest, { mode: 0o700 }));
});
