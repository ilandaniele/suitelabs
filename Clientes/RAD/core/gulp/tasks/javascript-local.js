...
const typeScriptCompilerOptions = {
    ...
    paths: {
        'schema-dts': ['schema-dts/dist/schema']
    },
    types: [],
    lib: ['DOM', 'ES5']
};
...
function getPathsPerModuleName(config) {
    ...
    paths.forEach(filePath => {
        ...
        if (modulePath.indexOf(package_manager.distro.folders.thirdPartyModules) === 0) {
            if (config && config.shim && config.shim[moduleName]) {
                const fileName = path.basename(modulePath);
                modulePath = path.join('javascript', 'compiled', fileName);
            } else {
                modulePath = modulePath.replace(package_manager.distro.folders.thirdPartyModules, 'third_parties');
            }
