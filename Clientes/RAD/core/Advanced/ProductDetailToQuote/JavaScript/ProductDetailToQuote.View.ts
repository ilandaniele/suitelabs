...
const ProductDetailToQuoteView: any = BackboneView.extend({
    ...
    // @method checkItemId Checks if the Item (using Item Id) to add in the
    // Quote List is already added
    // @param {ProductList.Model} productListLineJson
    // @param {ProductList.Model} productListLineAux
    // @return {Boolean}
    checkItemId: function(productsInQuoteList, productItem) {
        let isItemId = false;
        if (
            productItem.item.matrixchilditems_detail &&
            productItem.item.matrixchilditems_detail.length
        ) {
            isItemId = !!_.find(productItem.item.matrixchilditems_detail, (matrixItem: any) => {
                return matrixItem.internalid === productsInQuoteList.item.internalid;
            });
        } else {
            isItemId =
                parseInt(productItem.item.internalid, 10) ===
                parseInt(productsInQuoteList.item.internalid, 10);
        }
        return isItemId;
    },
    // @method Checks if the Item (using Item Options) to add in the
    // Quote List is already added
    // @param {ProductList.Model} productListLineJson
    // @param {ProductList.Model} productListLineAux
    // @return {Boolean}
    checkItemOptions: function(productsInQuoteList, productItem) {
        let index = 0;
        let result = true;
        while (
            productsInQuoteList.options &&
            index < productsInQuoteList.options.length &&
            result
        ) {
            const productListLineJsonOptions =
                productsInQuoteList.options[index] && productsInQuoteList.options[index].value;
            const productListItemOptions =
                productItem.options[index] && productItem.options[index].value;
            if (!_.isEqual(productListLineJsonOptions, productListItemOptions)) {
                result = false;
            }
            index++;
        }
            ...
            this.application.ProductListModule.Utils.getRequestAQuoteProductList().done(function(
                ...
                if (!product_list_json.internalid) {
                    ...
                    const item_present_in_list = _.find(product_list_json.items, function(
                        ...
                        return (
                            self.checkItemId(product_list_line_json, product_list_line_aux) &&
                            self.checkItemOptions(product_list_line_json, product_list_line_aux)
                        );
                    });
