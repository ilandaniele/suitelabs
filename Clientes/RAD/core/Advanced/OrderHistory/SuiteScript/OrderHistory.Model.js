    ...
    return Transaction.extend({
        ...
        setExtraListFilters: function() {
            ...
            if (
                ...
                this.filters.email = ['email', 'contains', email];
            }
        },
        ...
        update: function(id, data, headers) {
            ...
            if (data.status === 'cancelled') {
                ...
                let cancelResponse = nlapiRequestURL(url, JSON.stringify(data), headers);
                let code = cancelResponse.getCode();
                // we should change this in the future to only use POST request
                // but, because we have Customers
                // with the Canary ERP_O2C_WATERLOO_CSRF_CHECK_BYPASS on
                // we need to make sure do not break their stores
                if (
                    code === 500 &&
                    cancelResponse
                        .getBody()
                        .toString()
                        .indexOf('INSUFFICIENT_PERMISSION') !== -1
                ) {
