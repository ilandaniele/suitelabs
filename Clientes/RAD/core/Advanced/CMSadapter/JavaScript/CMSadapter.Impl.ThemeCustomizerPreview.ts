...
export class CMSAdapterImplThemeCustomizerPreview {
    ...
    public listenForCMS() {
        ...
        if (SC.ENVIRONMENT.isExtended && SC.ENVIRONMENT.embEndpointUrl) {
            SC._sassCompilerPromise = jQuery.Deferred();
            this.CMS.on('theme:config:get', (promise: CMSPromise) => {
                this.application.getLayout().once('afterAppendView', () => {
                    const scriptImpormap = document.createElement('script');
                    scriptImpormap.type = 'importmap';
                    scriptImpormap.text = `
						{
							"imports": {
								"immutable": "${Utils.getAbsoluteUrl('javascript/immutable.es.js')}",
								"sass": "${Utils.getAbsoluteUrl('javascript/sass.default.js')}"
							}
						}
					`;
                    document.body.appendChild(scriptImpormap);
                    ...
                    scriptModule.text = `
						...
						SC._sassCompilerPromise.resolve(sass);
					`;
    ...
    private static extractVariablesMetadata(css: string) {
        ...
        let values = metadata.substring(0, end);

        // Fix for Themes that have font-awesome v4.7.0 or less
        // with values in _variables.scss like $fa-var-500px: "\f26e"; or $fa-var-500px: '\f26e';
        // instead of $fa-var-500px: \f26e;
    ...
    private buildDummyCSS(sassStr) {
        ...
        try {
            this.getSassCompiler().done(sassCompiler => {
                const compiledSass = sassCompiler.compileString(sassStr);
                promise.resolve(compiledSass.css);
            });
        } catch (e) {
            promise.reject(e ? e : 'Unexpected error compiling front-end SASS variables');
        }
    ...
    private getSassCompiler() {
        return Environment.getSC()._sassCompilerPromise;
    }
