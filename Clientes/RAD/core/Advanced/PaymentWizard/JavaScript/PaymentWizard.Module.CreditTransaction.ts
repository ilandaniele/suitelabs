...
const PaymentWizardModuleCreditTransaction: any = WizardStepModule.extend({
    ...
    events: {
        'click [data-action="select"]': 'toggleTransactionHandler',
    ...
    toggleTransactionHandler: function(e) {
        const $target = this.$(e.currentTarget).closest('[data-action="transaction"]');

        if ($target.hasClass('disabled')) {
            return;
        }

        this.toggleTransaction(e.currentTarget.value);
    },
