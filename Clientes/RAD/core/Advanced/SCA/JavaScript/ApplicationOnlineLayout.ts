...
export class ApplicationOnlineLayout extends ApplicationLayout {
    ...
    protected processJsonLd(): void {
        ...
        const SEO = 'seo';
        const currentJsonLd = jQuery('head script[type="application/ld+json"]');

        if (currentJsonLd.data('generated') === SEO) {
            currentJsonLd.data('generated', '');
        } else {
            ...
            jQuery.when(...promises).then(
                ...
                (...jsonLds: JSONArray): void => {
                    ...
                    if (!_.isEmpty(fullJsonLd)) {
                        ...
                        jQuery(
                            `<script type="application/ld+json" data-generated="${
                                SC.isPageGenerator() ? SEO : ''
                            }">${JSON.stringify(fullJsonLd)}</script>`
                        ).appendTo('head');
                    }
                }
            );
