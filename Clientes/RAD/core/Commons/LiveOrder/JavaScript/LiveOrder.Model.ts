...
const LiveOrderModel = TransactionModel.extend(
    {
        ...
        removeAllLines: function() {
            return this.cancelableTrigger('before:LiveOrder.removeAllLines').then(() => {
                this.sync('delete', this, {
                    killerId: AjaxRequestsKiller.getKillerId(),
                    validate: false
                }).done(attributes => {
                    this.set(attributes);
                    this.cancelableTrigger('after:LiveOrder.removeAllLines');
                });
            });
        },
        // @method failActions invoked when the save promise fails in submit
        // @param {String} actionId
        // @return {Void}
        failActions: function(actionId) {
            const loggers = Loggers.getLogger();
            loggers.end(actionId, {
                operationIds: this.getOperationIds(),
                status: 'fail'
            });
            this.set('internalid', 'cart');
        },
        // @method getOutOfStockErrorMessage returns the error message
        // showing the items that are out of stock
        // @return {String}
        getOutOfStockErrorMessage: function() {
            const outOfStockItems = this.get('confirmation').get('outofstockitems');
            const lines = this.get('lines');
            const showItemRemainingQuantity = Configuration.get(
                'checkoutApp.showItemRemainingQuantity'
            );
            let message = Utils.translate(
                'Your order cannot be processed because there is insufficient stock for the following:',
            ...
            lines.each(line => {
                ...
                outOfStockItems.forEach(outOfStockItem => {
                    if (outOfStockItem.internalid === item.get('internalid')) {
                        message += `<li>- ${item.get('_name')} ${
                            showItemRemainingQuantity
                                ? Utils.translate(
                                      '($(0) item/s available in stock).',
                                      outOfStockItem.availablequantity,
            ...
            message += Utils.translate(
                'Please reduce the quantity and order again.',
                'Out of stock item error'
            );
            return message;
        },
        // @method checkIfOutOfStockError checks if the error is caused by an out of stock error
        // @return {Boolean}
        checkIfOutOfStockError() {
            return (
                this.get('confirmation').statuscode === 'error' &&
                this.get('confirmation').reasoncode === 'ERR_WS_INVENTORY' &&
                this.get('confirmation').outofstockitems.length > 0
            );
        },
        ...
        submit: function submit() {
            ...
            const submitPromise = jQuery.Deferred();
            ...
            return this.cancelableTrigger('before:LiveOrder.submit').then(function() {
                ...
                self.save()
                    .done(function(order) {
                        if (self.checkIfOutOfStockError()) {
                            self.failActions(actionId);
                            ...
                            const orderOperationIds = self.getLatestOperationIds(
                                lastOperationIdIndex
                            );
                            loggers.end(actionId, {
                            ...
                            const suiteTaxEnabled = Configuration.get(
                                'siteSettings.isSuiteTaxEnabled'
                            );
