...
define('LiveOrder.Model', [
    ...
    return SCModel.extend({
        ...
        submit: function submit(threedsecure) {
            ...
            confirmation = ModelsInit.order.submit({ showoutofstockdetails: 'T' });
        ...
        backFromPayPal: function backFromPayPal() {
            ...
            if (
                ...
                if (customer_values.customfields) {
                    const customfields = {};
        ...
        // @method removeAllLines
        removeAllLines: function() {
            ModelsInit.order.removeAllItems();
            ModelsInit.context.setSessionObject('lines_sort', []);
        },

        // @method areLineOptionsEquals
        // @param {LiveOrder.Model.Line} newLine
        // @param {LiveOrder.Model.Line} originalLineObject
        areLineOptionsEquals: function areLineOptionsEquals(newLine, originalLineObject) {
            const parsedOptions = this.parseLineOptionsFromCommerceAPI(originalLineObject.options);
            const newLineOptionsLength = newLine.options?.length ? newLine.options?.length : 0;
            const originalLineObjectOptionsLength = originalLineObject.options?.length
                ? originalLineObject.options?.length
                : 0;
            let result = newLineOptionsLength === originalLineObjectOptionsLength;
            _.each(newLine.options, function(option) {
                if (result) {
                    const originalOption = _.find(parsedOptions, function(availableOption) {
                        return option.cartOptionId === availableOption.cartOptionId;
                    });
                    const isSameId = option.value?.internalid === originalOption?.value?.internalid;
                    // for non-mandatory option, if original line doesn't have option and new
                    // line option doesn't have value, then its equal.
                    const areBothEmpty = !originalOption && !option.value;

                    if (!(isSameId || areBothEmpty)) {
                        result = false;
                    }
                }
            });
        ...
        updateLine: function updateLine(lineId, lineData) {
            const originalLineObject = ModelsInit.order.getItem(lineId, [
                'quantity',
                'internalid',
                'options',
                'fulfillmentPreferences'
            ]);
            if (!originalLineObject || !originalLineObject.internalid) {
                return;
            }

            this.storePromosPrevState();
            if (
                originalLineObject.orderitemid === lineData.internalid &&
                originalLineObject.internalid == lineData.item.internalid
            ) {
                if (
                    _.isNumber(lineData.quantity) &&
                    lineData.quantity > 0 &&
                    lineData.quantity !== originalLineObject.quantity
                ) {
                    lineData.orderitemid = lineData.internalid;
                    ModelsInit.order.updateItemQuantity(lineData);
                }
                if (
                    this.isPickupInStoreEnabled &&
                    (lineData.fulfillmentChoice !==
                        originalLineObject.fulfillmentPreferences.fulfillmentChoice ||
                        lineData.location !== originalLineObject.location)
                ) {
                    const hasLocationData =
                        lineData.fulfillmentChoice === 'pickup' && lineData.location;
                    const fulfillmentPreferences = {
                        fulfillmentChoice: lineData.fulfillmentChoice,
                        pickupLocationId: hasLocationData
                            ? parseInt(lineData.location, 10)
                            : undefined
                    };
                    ModelsInit.order.updateItemFulfillmentPreferences({
                        orderitemid: lineData.internalid,
                        fulfillmentPreferences: fulfillmentPreferences
                    });
                    if (this.isMultiShippingEnabled && lineData.fulfillmentChoice !== 'pickup') {
                        // Sets its ship address (if present)
                        lineData.shipaddress &&
                ...
                if (!this.areLineOptionsEquals(lineData, originalLineObject)) {
                    ModelsInit.order.setItemOptions(
                        lineId,
                        this.parseLineOptionsToCommerceAPI(lineData.options)
                    );
                }
            } else {
                let linesSort = this.getLinesSort();
                const currentPosition = _.indexOf(linesSort, lineId);
                ...
                try {
                    ...
                    if (originalLineObject.options && originalLineObject.options.length) {
                        ...
                        _.each(originalLineObject.options, function(option) {
        ...
        getPaymentMethods: function getPaymentMethods(order_fields) {
            ...
            } else if (payment && payment.ach && payment.ach.paymentmethod) {
                const { ach } = payment;
                paymentmethods.push({
