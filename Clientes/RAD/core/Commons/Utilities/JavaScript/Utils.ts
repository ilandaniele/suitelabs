...
export function formatCurrency(
    ...
    let valueFloat: number = parseFloat(<string>value);

    if (isNaN(valueFloat)) {
        return <string>value;
    }

    const negative: boolean = valueFloat < 0;
    const settings: SiteSettings = (SC && SC.ENVIRONMENT && SC.ENVIRONMENT.siteSettings) || {};
    const currentCurrency: Currency =
        (SC && SC.ENVIRONMENT && SC.ENVIRONMENT.currentCurrency) || {};
    const currencies = [
        currentCurrency,
        ...((SC && SC.ENVIRONMENT && SC.ENVIRONMENT.availableCurrencies) || [])
    ];
    ...
    const currencySymbol = symbol ? symbol : currentCurrency.symbol || '$';
    let beforeValue = true;

    const currencyBySymbol: any = _.find(currencies, cur => {
        return cur.symbol === symbol;
    });

    if (currencyBySymbol) {
        beforeValue = (currencyBySymbol.symbolplacement || 1) === 1;
    }

    valueFloat = parseInt(`${(Math.abs(valueFloat) + 0.005) * 100}`, 10) / 100;

    let valueString: string = valueFloat.toString();

    valueString = valueString.replace('.', decimalseparator);
    let decimalPosition: number = valueString.indexOf(decimalseparator);
    ...
    const aux = beforeValue ? currencySymbol + thousandString : thousandString + currencySymbol;
    valueString = negative ? negativeprefix + aux + negativesuffix : aux;
...
function isUrlImage(arg: UrlImage | ImageContainer): arg is UrlImage {
    return 'url' in arg && 'altimagetext' in arg;
}

export function imageFlatten(images: ImageContainer | UrlImage): UrlImage[] {
    const result: UrlImage[][] = [];
    if (isUrlImage(images)) {
        return [images];
    }
