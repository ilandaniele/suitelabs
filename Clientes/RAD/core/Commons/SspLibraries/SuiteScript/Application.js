            ...
            getEnvironment: function getEnvironment(request) {
                ...
                // The currency of the current user is valid fot this host let's just use that
                    ...
                    if (!result.currentCurrency) {
                        result.currentCurrency = result.availableCurrencies[0];
                    }
                }
                ...
                // If setShopperCurrency has no effect, it means that the shopper already made a purchase
                // Lets use that currency regardless if its not available for the current domain
                result.currentCurrency = _.find(siteSettings.currencies || [], function(currency) {
                    return currency.code === ModelsInit.session.getShopperCurrency().code;
                });
