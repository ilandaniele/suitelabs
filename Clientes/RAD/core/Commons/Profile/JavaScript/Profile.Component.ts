        ...
        return SCBaseComponent.extend({
            ...
            getCookieOptions: function(): JQuery.Deferred<CookieOptions[]> {
                try {
                    ...
                    let copiedOptions = Utils.deepCopy(profileModelInstance.get('cookieoptions'));
                    if (copiedOptions) {
                        copiedOptions.consentLastUpdate = profileModelInstance.get('consentlastupdate');
                    }
                    return jQuery.Deferred().resolve(copiedOptions);
                } catch (error) {
                    return jQuery.Deferred().reject(error);
                }
            },
