...
const BackboneFormView: any = {
    ...
    // @return {Void}
    synchronizeModels: function(view_model, options) {
        if (!view_model) {
            return;
        }
        ...
        _.each(changed_attributes, (value, attribute) => {
            if (options.unset) {
                this.validationModel.unset(attribute);
            } else {
                this.validationModel.set(attribute, value);
            }
        });
    },
