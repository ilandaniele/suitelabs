...
export = BackboneView.extend({
    ...
    addToCart: function addToCart(e) {
        ...
        if (!this.model.isNew() && this.model.get('source') === 'cart') {
            this.trackAddOrRemoveItemEvent(this.model, this.cart);
    ...
    trackAddOrRemoveItemEvent: function trackAddOrRemoveItemEvent(newLine, cart) {
        const oldLine = _.find(cart.get('lines').models, function(line: any) {
            return line.get('internalid') === newLine.get('internalid');
        });
        const difference = oldLine ? newLine.get('quantity') - oldLine.get('quantity') : newLine.get('quantity');
        cart.trackItemQuantityChangeEvent(newLine, difference);
    }
});
