	...
	<div class="requestquote-wizard-module-comments">
		<div class="requestquote-wizard-module-comments-box requestquote-wizard-module-payment-method-box">
			...
			{{else}}
				...
				<select name="select-division" class="list-header-view-accordion-body-select requestquote-wizard-half-box-write-field" data-action="select-division">
					{{#each divsions}}
						<option value="{{divisionId}}" {{#if selected}} selected {{/if}}>{{divisionValue}}</option>
					{{/each}}
				</select>
			{{/if}}
		</div>
