		...
		<div class="requestquote-wizard-module-comments-box requestquote-wizard-module-payment-method-box">
			...
			{{#if isReadOnly}}
				<div class="requestquote-wizard-module-comments-box-message requestquote-wizard-half-box-read-field">
					<p>{{selectedPreferredCarrierName}}</p>
				</div>
			{{else}}
				<div data-type="alert-placeholder-module"></div>
				<select name="select-preferred-carrier" class="list-header-view-accordion-body-select requestquote-wizard-half-box-write-field" data-action="select-preferred-carrier">
					{{#each preferredCarriers}}
						<option value="{{preferredCarrierId}}" {{#if selected}} selected {{/if}}>{{preferredCarrierName}}</option>
					{{/each}}
				</select>
				<div class="shipping-message-wrap">{{{shippingrestrictionmessage}}}</div>
			{{/if}}
		</div>
	</div>
{{/if}}