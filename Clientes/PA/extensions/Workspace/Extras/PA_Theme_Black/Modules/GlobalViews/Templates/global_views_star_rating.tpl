<div class="global-views-star-rating" data-validation="control-group">
	<div class="global-views-star-rating-container">
		...
		<div id="{{name}}" class="global-views-star-rating-area {{#if isReviewMode}}global-views-star-rating-area-review-mode{{/if}}" data-toggle='rater' data-validation="control" data-name="{{name}}" data-max="{{maxValue}}" data-value="{{value}}">
			...
			{{#if isAggregateOrReview}}{{#unless isPLP}}
		...
		{{#if showValue}}
			<span class="global-views-star-rating-value" {{#if isAggregateOrReview}} {{#unless isPLP}}itemprop="ratingValue"{{/unless}} {{/if}}>
				{{value}}
			</span>
		{{else}}
			{{#if isAggregateOrReview}}{{#unless isPLP}}<meta itemprop="ratingValue" content="{{value}}">{{/unless}}{{/if}}
		{{/if}}
		...
		{{#if showRatingCount}}
			<span class="global-views-star-rating-review-total">
				...
				{{#if ratingCountGreaterThan0}}
					<span class="global-views-star-rating-review-total-number" {{#if isAggregateArea}} {{#unless isPLP}}itemprop="reviewCount"{{/unless}} {{/if}}>{{ratingCount}}</span>
					{{#if hasOneReview}}
					...
					{{/if}}
				{{else}}
					<span class="global-views-star-rating-review-total-empty-number" {{#if isAggregateArea}} {{#unless isPLP}}itemprop="reviewCount"{{/unless}} {{/if}}>({{ratingCount}})</span>
					<span class="global-views-star-rating-review-total-no-review">{{ translate ' No Reviews yet'}}</span>
				{{/if}}
			</span>
		{{else}}
			{{#if isAggregateArea}}{{#unless isPLP}}<meta itemprop="reviewCount" content="{{ratingCount}}">{{/unless}}{{/if}}
		{{/if}}
	</div>
</div>
