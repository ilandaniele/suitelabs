{{#if isCountryAndStatePresent}}
	...
	<div  class="global-views-states-group-form-controls" data-validation="control">
		<select class="{{inputClass}} global-views-states-group-select" id="{{manage}}state" name="state" data-type="selectstate" data-action="selectstate" {{#if isCheckout}}data-field="checkout_state"{{/if}}>
			<option value="">
				{{translate '-- Select --'}}
			</option>
			{{#each states}}
...
{{else}}
	...
	<div  class="global-views-states-group-form-controls" data-validation="control">
		<input
			...
			{{#if isCheckout}}data-field="checkout_state"{{/if}}
		>
	</div>
{{/if}}
