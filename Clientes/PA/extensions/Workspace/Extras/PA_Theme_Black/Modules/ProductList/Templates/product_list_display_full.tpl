<tr class="{{#if isChecked}}active{{/if}}" data-id="{{lineId}}" data-item-id="{{itemId}}"{{#if showCheckbox}}data-action="product-list-item"{{/if}}>
	...
	<td class="product-list-display-full-thumnail">
		<img src="{{brightedgeImageDomain 'noWebsiteDomain' 'thumbnail'}}{{resizeImage thumbnail.url 'thumbnail' 'removeImageUrlProtocol'}}" alt="{{thumbnail.altimagetext}}">
	</td>

	<td class="product-list-display-full-details">
		<p class="product-list-display-full-name">
			<a class="product-list-display-full-name-anchor" {{{linkAttributes}}}>
				{{#if pagetitle}}
					{{pagetitle}}
				{{else}}
					{{productName}}
