<div class="order-wizard-confirmation-module alert fade in">
	...
	{{#if isMultiOrder}}
		<p class="order-wizard-confirmation-module-body" name="orderNumber">{{translate 'Your order numbers are'}}
	{{else}}
	...
	{{/if}}
      ...
      {{#if isGuestAndCustomerCenter}}
			  {{#if isMultiOrder}}
				  {{#each OrderIdObjs}}
					  <a href="#" data-touchpoint="customercenter" data-hashtag="#/purchases/view/salesorder/{{internalid}}">{{tranid}}{{#if @last}}.{{else}},{{/if}}</a> 
				  {{/each}}
			  {{else}}
	...
	{{#if isMultiOrder}}
		<p class="order-wizard-confirmation-module-body">{{translate 'We have received your orders and will process it right away.'}}</p>
	{{else}}
		<p class="order-wizard-confirmation-module-body">{{translate 'We have received your order and will process it right away.'}}</p>
	{{/if}}
	...
	{{#if isMultiOrder}}
		{{#each OrderIdObjs}}
			<a href="{{pdfUrl}}" target="_blank" class="order-wizard-confirmation-module-download-pdf">
				<i class="fa fa-file-pdf-o fa-6 order-wizard-confirmation-module-download-pdf-icon" aria-hidden="true"></i> {{ tranid }}
			</a> 
		{{/each}}
	{{else}}
		<a href="{{pdfUrl}}" target="_blank" class="order-wizard-confirmation-module-download-pdf">
