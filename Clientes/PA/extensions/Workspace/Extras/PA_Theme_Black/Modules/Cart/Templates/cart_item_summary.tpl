{{#if isPriceEnabled}}
				...
				{{#if enableCartPageValidations}}
					{{#if couponApplied}}
						{{#if containsItemCoupon}}
							<div class="promotion-info {{itemCouponDiscountType}}">
	...
	<div class="cart-item-summary-item-list-actionable-amount">
			{{#if containsItemCouponDiscount}}
				{{#if enableCouponValidations}}
					{{#if enableCartPageValidations}}				
						<span class="cart-item-summary-item-list-actionable-amount-label">{{translate 'Discounted Amount:  ' }}</span>	
						<span class="cart-item-summary-amount-value">${{ finalItemPriceAfterDiscount }}</span>		
					{{/if}}
				{{/if}}		
			{{else}} 
				<span class="cart-item-summary-item-list-actionable-amount-label">{{{cartLineItemAmountLabel}}}:</span>
	...
	{{#if showStandardPrice}}
		<div class="cart-item-summary-item-list-actionable-amount">
			<span class="cart-item-summary-item-list-actionable-amount-label">{{translate 'Standard Price:  ' }}</span>	
			<span class="cart-item-summary-amount-value">{{{ baseQtyOnePrice }}}</span>	
		</div>
	{{/if}}
	{{#unless isRetailDeliveryItem}}
