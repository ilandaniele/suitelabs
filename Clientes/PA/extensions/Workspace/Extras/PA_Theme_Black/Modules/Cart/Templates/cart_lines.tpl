<div id="{{lineId}}" data-item-id="{{itemId}}" data-type="order-item" class="{{#if showGeneralClass}} {{generalClass}} {{/if}} cart-lines-row">
	...
	<div class="cart-lines-table-middle">
		...
		{{#if isNavigable}}
			<a {{{linkAttributes}}} class="cart-lines-name-link">
				{{#if item.pagetitle}} 
					{{item.pagetitle}} 
				{{else}} 
					{{#if item._matrixParent.pagetitle}}  
						{{item._matrixParent.pagetitle}}
					{{else}}  
						{{item._name}} 
					{{/if}}
				{{/if}}
			...
			{{#if isRestrictionMessageEnabled}}
				{{#unless isRestrictionMessageEmpty }}
					<div class="restriction-message">
						<div class="item-views-cell-actionable-restriction-message">
							<div class="item-details-message">
								<a class="item-details-message-head collapsed" data-toggle="collapse" data-target="#{{itemId}}" aria-expanded="false" aria-controls="{{itemId}}">
									<i class="fa fa-exclamation-triangle"></i> {{{restrictionMessageTitle}}}
									<i class="acordion-head-toggle-icon"></i>
								</a>
							</div>
							<div class="item-details-message-content collapse" id="{{itemId}}" role="tabpanel" >
		...
		<div class="cart-lines-stock-eta-wrapper">
			<div class="cart-lines-stock" data-view="Product.Stock.Info"></div>
			<div class="cart-lines-eta-msg-wrapper" data-view="ETA.Message"></div>
		</div>
