...
<div data-view="Promocode.Notifications"></div>
	{{#if enableCouponValidations}}
		{{#if enableCartPageValidations}}
			{{#if enableCouponExclusionTooltip}}
				{{#if containsItemCoupon}}
					{{#if couponApplied}}
						{{#if_eq promocodesDiscountTotal 0}}
							<div class="promo-code-toll-wrapper">
								<div class="cart-summary-expander-head">
									<a class="cart-summary-expander-head-toggle collapsed" data-toggle="collapse" data-target="#eligible-brand-container" aria-expanded="false" aria-controls="eligible-brand-container">
...
<div class="cart-summary">
	...
	<div class="cart-summary-free-shipping" data-view="FreeShipping.Info"></div>
		...
		{{#if isPriceEnabled}}
			<div class="cart-summary-subtotal">
				...
				{{/if}}
				<div class="credova-cart-container">
					<div data-view="Widget.Cart.Credova"></div>
				</div>
			</div>
			...
			{{#if enableCouponValidations}}
				{{#if enableCartPageValidations}}
					{{#if promocodesDiscountTotal}}
						{{#if containsItemCoupon}}
							{{#if couponApplied}}
								{{#unlessEquals promocodesDiscountTotal 0.00}}
								<div class="cart-summary-discount-applied">
									<p class="cart-summary-grid-float">
										<span class="cart-summary-amount-discount-total">
											${{promocodesDiscountTotal}}
										</span>
										{{translate 'Discount Applied (Item Coupon)'}}
									</p>
								</div>
								{{/unlessEquals}}
							{{/if}}
	...
	{{#if showGOVDiscountMessage}}
		<div class="gov-discount-message-box discount-cart-summary">
	...
	{{#if showActions}}
		<div class="cart-summary-button-container">
			{{#if prevent_checkout}}
				<div class="cart-summary-inventory-validation-messages">{{{prevent_checkout_message}}}</div>
			{{else}}
...
{{#if promoCodePlaceholderText}}
	<div class="promo-code-placeholder-text">
		<i class="fa fa-info-circle"></i>
		{{promoCodePlaceholderText}}
	</div>
{{/if}}
