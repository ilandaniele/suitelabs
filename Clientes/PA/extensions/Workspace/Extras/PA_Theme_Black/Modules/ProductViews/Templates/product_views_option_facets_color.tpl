				...
				{{#if internalId}}
					<div class="product-views-option-facets-color-picker-small">
						<label class="product-views-option-facets-color-picker-label">
								<span class="product-views-option-facets-color-picker-anchor" onclick="window.open('{{url}}'); event.stopPropagation(); return false;">
									{{#if isColorTile}}
									...
									{{else}}
										<img data-label="label-{{../cartOptionId}}" value="{{internalId}}"
											src="{{brightedgeImageDomain 'noWebsiteDomain' 'nothumb' image}}{{removeImageUrlProtocol image.src}}"
