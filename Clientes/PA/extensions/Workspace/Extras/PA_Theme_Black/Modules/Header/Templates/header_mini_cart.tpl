<a class="header-mini-cart-menu-cart-link {{#if showLines}}header-mini-cart-menu-cart-link-enabled{{/if}}" data-type="mini-cart" title="{{translate 'Cart'}}" data-touchpoint="{{cartTouchPoint}}" data-hashtag="#cart" href="#">
	...
	<img class="pa-icon visible-xs-block shopping-cart-icon" src="{{getThemeAssetsPathWithDefault cartIcon 'img/Icons/icon-ionic-ios-cart.svg'}}" alt="Register"  width="26" height="26">	
	
	<span class="item-count">{{itemsInCart}}</span>
...
<div class="header-mini-cart {{#unless showLines}}empty{{/unless}}">
	 {{#if showLines}} 
	 	<div class="header-mini-cart-container">
			<ul data-view="Header.MiniCartItemCell"></ul>
			{{#if prevent_checkout}}
		...
		<div class="header-mini-cart-buttons">
			...
			<div class="header-mini-cart-buttons-right">
				<a class="{{#if prevent_checkout}}header-mini-cart-button-checkout disabled{{else}}header-mini-cart-button-checkout{{/if}}" {{#unless prevent_checkout}}href="#" data-touchpoint="checkout" data-hashtag="#" data-action="checkout{{/unless}}">
					{{translate 'Checkout'}}
				</a>
			</div>
		</div>
