...
{{else}}
	...
	{{#if hasResults}}
		<!-- <div class="itemssearcher-item-all-results sc-itemssearcher-item-all-results">
			{{translate 'See all'}}
	...
	{{else}}
		{{#if isAjaxDone}}
			<div class="itemssearcher-item-no-results">
				<div class="wrapper-noresult-message">
					<div class="wrapper-icon-noresults">
						<!-- <img class="icon-noresults" src="{{ getExtensionAssetsPath 'img/icon-search-load.png' }}" > -->
					</div>
				...
				<span class="hide">{{currentQuery}}</span>
				<div class="categories-noresults" data-view="Categories.NoResults"></div>
			</div>
		{{else}}
