    ...
    return TransactionModel.extend({
        ...
        update: function(salesorder_id, quote_id, data) {
            ...
            if (quote_id) {
                ...
                const backupShippingCost = this.record.getFieldValue('shippingcost');

                let shipOrBillError = 'shipping';
        ...
        getBillAddress: function() {
            const quote = this.getQuoteOrderSessionData();
            return (quote && quote.billaddress) || '';
        },
        ...
        getRecordAddresses: function() {
            ...
            this.result.billaddress = this.addAddress({
                internalid:
                    this.record.getFieldValue('billaddresslist') || this.getBillAddress() || '',
        ...
        setQuoteOrderSessionData: function setQuoteOrderData(quoteOrderData) {
            if (quoteOrderData && quoteOrderData.paymentmethods) {
                ModelsInit.context.setSessionObject(
                    'quoteOrderData',
                    typeof quoteOrderData === 'object'
                        ? JSON.stringify(quoteOrderData)
                        : quoteOrderData
                );
            }
        },

        getQuoteOrderSessionData: function getQuoteOrderData() {
            const quoteOrderData = ModelsInit.context.getSessionObject('quoteOrderData');
            return typeof quoteOrderData === 'object' ? quoteOrderData : JSON.parse(quoteOrderData);
        },

        // @method submit Saves the current record
        // @return {Transaction.Model.Confirmation}
        submit: function submit(threeDSecure, quoteData) {
            if (quoteData) {
                this.setQuoteOrderSessionData(quoteData);
            }

            const quoteOrderData = quoteData || this.getQuoteOrderSessionData();
            const paymentMethod =
                quoteOrderData && quoteOrderData.paymentmethods && quoteOrderData.paymentmethods[0];
            ...
            if (
                ...
                return this.process3DSecure(quoteOrderData || quoteData, paymentMethod);
            }

            this.preSubmitRecord();

            this.data = this.data ? this.data : quoteOrderData;
            let new_record_id;
            ...
            if (threeDSecure) {
                this.update(this.data.salesorder_id, this.data.quote_id, this.data);
                const order_info = this.get(this.data.salesorder_id, this.data.quote_id, true);
                ...
                order_info.confirmation = this.postSubmitRecord({ internalid: new_record_id });
        ...
        getConfirmation: function getConfirmation(internalid) {
            ...
            try {
                confirmation = nlapiLoadRecord('salesorder', confirmation.internalid);
            } catch (e) {
        ...
        submitRecord: function(paymentRecord, data) {
            const newSalesOrderRecord = nlapiSubmitRecord(this.record);
            newSalesOrderRecord.confirmation = data;
            return newSalesOrderRecord;
        },
        ...
        process3DSecure: function process3DSecure(data, paymentMethod) {
            ...
            if (this.isCardHolderAuthenticationSupported) {
                ...
                this.authenticationOptions = {
                    amount: this.record.getFieldValue('total'),
                    paymentOption: this.record.getFieldValue('paymentoption'),
                    paymentProcessingProfile: this.record.getFieldValue('paymentprocessingprofile'),
                    notificationURL: `${notificationURL}?n=${site_id}&type=${3}`
                };
                ...
                switch (cardHolderAuthenticationRecord.getValue('status')) {
                    ...
                    case this.CHA_STATUS.AUTHENTICATED:
                        ...
                        return this.submitRecord(this.record, data).confirmation;

                    case this.CHA_STATUS.WAITING_FOR_DEVICE_AUTHENTICATION:
                        this.setQuoteOrderSessionData(data);
